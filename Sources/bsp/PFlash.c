...
void Flash_Init(void)
{
    ...
    FCLKDIV = 0x0F;    /*OSCCLK = 16MHz, FDIV[6:0] is recommanded as F; */
...
ERROR_TYPE PFlash_Program(UINT32 address, UINT16 *ptr)
{
  UINT8 i;	  
  DisableInterrupts;
...
/******************************************************************************/
#pragma CODE_SEG RAM_CODE
ERROR_TYPE PFlash_EraseSector(UINT32 address)
{
  while((FSTAT & FSTAT_CCIF_MASK) == 0);   //wait if command in progress
  FSTAT = 0x30;             //clear ACCERR and PVIOL
...
/******************************************************************************/
#pragma CODE_SEG RAM_CODE
ERROR_TYPE PFlash_EraseSectorBySector(UINT32 addr_l, UINT32 addr_h)
{ 
  UINT32 Address;
  ERROR_TYPE Error;
